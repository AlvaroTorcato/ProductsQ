package com.example.productq.repositories;
...
public interface ProductRepository extends JpaRepository<Product,String> {
    @Query("select new com.example.productq.model.ProductDTO(f.sku,f.name) from Product f")
    Page<ProductDTO> findAllProductsFromCatalog(Pageable paging);

    @Query("select new com.example.productq.model.ProductDetailsDTO(f.sku,f.name,f.description) from Product f where f.sku = :sku")
    ProductDetailsDTO findProductBySku(@Param("sku") String sku);

    @Query("select new com.example.productq.model.ProductDTO(f.sku,f.name) from Product f where f.sku like %:nameOrSku% or f.name like %:nameOrSku%")
